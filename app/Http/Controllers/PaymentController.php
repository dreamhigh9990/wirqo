...
class PaymentController extends AccountBaseController
{
	...
	public function paymentsImportFiles(StoreRequest $request){
        ...
        while (($getData = fgetcsv($file, 10000, ",")) !== FALSE)
        {
			...
			if (!is_null($getData[0])) {
				...
				$getData[0] = $this->company->currency_id;
			}

			if ($getData[1] != '') {
				$project = Project::findOrFail($getData[1]);
				$payment->project_id = $getData[1];
				$payment->currency_id = $project->currency_id;
			}

			if ($getData[2] != '') {
				$invoice = Invoice::findOrFail($getData[2]);
				...
				if ($getData[3] > $invoice->amountDue()) {
					return Reply::error(__('messages.invoicePaymentExceedError'));
				}
			}

			$payment->default_currency_id = company()->currency_id;
			$payment->exchange_rate = $getData[4];
			$payment->amount = round($getData[5], 2);
			$payment->gateway = $getData[6];
			$payment->transaction_id = $getData[7];
			$payment->offline_method_id = $getData[8];
			$payment->paid_on = data('Y-m-d',strtotime($getData[9]));

			$payment->remarks = $getData[10];

			$payment->status = 'complete';
			$payment->bank_account_id = $getData[11];
			$payment->save();
	   }
       fclose($file);
