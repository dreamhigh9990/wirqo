...
class ProposalController extends AccountBaseController
{
	...
	public function proposalImportFiles(StoreRequest $request){
		
		$file = fopen($request->import, "r");
        while (($getData = fgetcsv($file, 10000, ",")) !== FALSE)
        {
			$proposal = new Proposal();
			$proposal->lead_id = $getData[0];
			$proposal->valid_till = data('Y-m-d',strtotime($getData[1]));
			$proposal->sub_total = $getData[2];
			$proposal->total = $getData[3];
			$proposal->currency_id = $getData[4];
			$proposal->note = trim_editor($getData[5]);
			$proposal->discount = round($getData[6], 2);
			$proposal->discount_type = $getData[7];
			$proposal->status = 'waiting';
			$proposal->signature_approval = ($getData[8]) ? 1 : 0;
			$proposal->description = trim_editor($getData[9]);
			$proposal->save();
	   }
       fclose($file);
