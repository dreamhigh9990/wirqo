...
class ExpenseController extends AccountBaseController
{
	...
	public function expensesImportFiles(StoreRequest $request){
        ...
        while (($getData = fgetcsv($file, 10000, ",")) !== FALSE)
        {
			$userRole = session('user_roles');
			$expense = new Expense();
			$expense->item_name = $getData[0];
			$expense->purchase_date = data('Y-m-d',strtotime($getData[1]));
			$expense->purchase_from = $getData[2];
			$expense->price = round($getData[3], 2);
			$expense->currency_id = $getData[4];
			$expense->category_id = $getData[5];
			$expense->user_id = $getData[6];
			$expense->default_currency_id = company()->currency_id;
			$expense->exchange_rate = $getData[7];
			$expense->description = trim_editor($getData[8]);
			$expense->bank_account_id = $getData[9];
			$expense->save();
	   }
       fclose($file);
