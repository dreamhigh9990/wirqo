...
class CreditNoteController extends AccountBaseController
{
	...
	public function creditnotesImportFiles(StoreRequest $request){
		
		$file = fopen($request->import, "r");
        while (($getData = fgetcsv($file, 10000, ",")) !== FALSE)
        {
			DB::beginTransaction();
			$invoice = Invoice::findOrFail($getData[0]);
			$clientId = null;
			if ($invoice->client_id) {
				$clientId = $invoice->client_id;
			}
			...
			$creditNote->cn_number = CreditNotes::count() + 1;
			$creditNote->invoice_id = $invoice->id;
			$creditNote->issue_date = $getData[1];
			$creditNote->due_date = $getData[2];
			$creditNote->sub_total = round($getData[3], 2);
			$creditNote->discount = round($getData[4], 2);
			$creditNote->discount_type = $getData[5];
			$creditNote->total = round($getData[6], 2);
			$creditNote->adjustment_amount = round($getData[7], 2);
			$creditNote->currency_id = $getData[8];
			$creditNote->save();
	   }
       fclose($file);
