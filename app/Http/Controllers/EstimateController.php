...
class EstimateController extends AccountBaseController
{
	...
	public function estimatesImportFiles(StoreRequest $request){
		dd('hered');
		die();	
        ...
        {
			$estimate = new Estimate();
			$estimate->client_id = $getData[0];
			$estimate->valid_till = data('Y-m-d',strtotime($getData[1]));
			$estimate->sub_total = round($getData[2], 2);
			$estimate->total = round($getData[3], 2);
			$estimate->currency_id = $getData[4];
			$estimate->note = trim_editor($getData[5]);
			$estimate->discount = round($getData[6], 2);
			$estimate->discount_type = $getData[7];
			$estimate->status = 'waiting';
			$estimate->description = trim_editor($getData[8]);
			$estimate->estimate_number = $getData[9];
			$estimate->save();
	   }
       fclose($file);
