...
class InvoiceController extends AccountBaseController
{
	...
	public function invoicesImportFiles(StoreRequest $request){
		
		$file = fopen($request->import, "r");
        while (($getData = fgetcsv($file, 10000, ",")) !== FALSE)
        {
			$invoice = new Invoice();
			$invoice->project_id = $getData[0] ?? null;
			$invoice->client_id = ($getData[1]) ?: null;
			$invoice->issue_date = data('Y-m-d',strtotime($getData[2]));
			$invoice->due_date = data('Y-m-d',strtotime($getData[3]));
			$invoice->sub_total = round($getData[4], 2);
			$invoice->discount = round($getData[5], 2);
			$invoice->discount_type = $getData[6];
			$invoice->total = round($getData[7], 2);
			$invoice->due_amount = round($getData[8], 2);
			$invoice->currency_id = $getData[9];
			$invoice->default_currency_id = company()->currency_id;
			$invoice->exchange_rate = $getData[10];
			$invoice->recurring = 'no';
			$invoice->billing_frequency = $getData[11] == 'yes' ? $getData[12] : null;
			$invoice->billing_interval = $getData[11] == 'yes' ? $getData[13] : null;
			$invoice->billing_cycle = $getData[11] == 'yes' ? $getData[14] : null;
			$invoice->note = trim_editor($getData[15]);
			$invoice->show_shipping_address = $getData[16];
			$invoice->invoice_number = $getData[17];
			$invoice->company_address_id = $getData[18];
			$invoice->estimate_id = $getData[19] ? $getData[19] : null;
			$invoice->bank_account_id = $getData[20];
			$invoice->payment_status = $getData[21] == null ? '0' : $getData[21];
			$invoice->save();
	   }
       fclose($file);
